...
"Phogat family?s real-life story."
    ...
    },
    {
        ...
        function playGame() {
            ...
            let score = 10; 
            let hintIndex = 0; 
            console.log("Welcome to Dumb Charades! Here You have to guess the movie name using the provided hints.");
            while (hintIndex < 10) {
                console.log(`Hint ${hintIndex + 1}: ${movie.hints[hintIndex]}`);
                const guess = prompt("Your guess: ").toLowerCase();
                const movieTitleLower = movie.title.toLowerCase();
                if (guess === movieTitleLower) {
                    console.log(`Congratulations! You guessed the movie '${movie.title}' correctly.`);
                    console.log(`Your final score is: ${score}`);
                    break;
                } else {
                    score--;
                    hintIndex++;
                    console.log("Incorrect guess! Better luck next time, here's another hint.");
                }
            }
            if (hintIndex === 10) {
                console.log("Try again next time!, You need to go and watch more movies I think, anyways the correct movie was:", movie.title);
            }
            const playAgain = prompt("Do you wish to play again? (yes/no): ").toLowerCase();
            if (playAgain === 'yes' || 'Yes') {
                playGame(); 
            } else {
                console.log("Thanks for playing Dumsaraj!");
            }
        }
        playGame();